...
async function main(): Promise<void> {
    ...
    (command === "recover" ||
      ...
      (command === "backup" &&
        (parsed.remaining.length === 0 ||
          parsed.remaining[0] === "create" ||
          parsed.remaining[0]?.startsWith("--") === true)) ||
      command === "uninstall" ||
  ...
  if (command === "install" || command === "recover") {
    ...
    if (parsed.remaining.length === 1 && parsed.remaining[0] === "--help") {
      console.log(
        "Usage: wikimemory backup [--deployment NAME|--local] [--output FILE]\n       wikimemory backup inspect|verify FILE"
      );
      return;
    }
    const { createBackup, inspectBackup } = await import("./archive.ts");
    const action = parsed.remaining[0];
    if (action === undefined || action === "create" || action.startsWith("--"))
      await createBackup(
        paths.record,
        paths.directory,
        action === "create" ? parsed.remaining.slice(1) : parsed.remaining
      );
    else if ((action === "inspect" || action === "verify") && parsed.remaining.length === 2)
      await inspectBackup(parsed.remaining[1] ?? "");
    else
      throw new Error(
        "Usage: wikimemory backup [--deployment NAME|--local] [--output FILE] | backup inspect|verify FILE"
      );
