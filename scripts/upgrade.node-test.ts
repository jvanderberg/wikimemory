...
await describe("packaged upgrade", async () => {
  ...
  await it("uses concise, user-facing upgrade and status summaries", () => {
    const upgrade = upgradeSummary(record, "0.2.9", "0.2.10", 0);
    ...
    assert.equal(readySummary("0.2.10"), "Wikimemory 0.2.10 is ready.\nDatabase: up to date.");
    assert.equal(
      readySummary("0.2.10", true),
      "Wikimemory 0.2.10 is already ready.\nDatabase: up to date."
    );
