...
describe("passkey boundaries", () => {
  ...
  it("serves setup without embedding setup material", async () => {
    ...
    expect(body).toContain("Set up your passkey");
    expect(body).not.toContain("Google");
  });

  it("rejects setup without the production bootstrap secret", async () => {
    const response = await exports.default.fetch(new Request("https://example.test/setup/options", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token: "a".repeat(32) })
    }));
    expect(response.status).toBe(403);
    await expect(response.json()).resolves.toEqual({ error: "This setup link is invalid or expired." });
  });

  it("enforces one-use bootstrap hashes and atomic challenge consumption", async () => {
    await env.DB.prepare("INSERT INTO passkey_bootstrap(used_token_hash, completed_at) VALUES ('hash-one', '2026-07-18T00:00:00Z')").run();
    await expect(env.DB.prepare("INSERT INTO passkey_bootstrap(used_token_hash, completed_at) VALUES ('hash-one', '2026-07-18T00:00:01Z')").run()).rejects.toThrow(/UNIQUE/u);
  ...
  it("generates origin-bound, user-verifying registration options for the current bootstrap", async () => {
    ...
    const response = await setupOptions(new Request("https://memory.example/setup/options", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token })
    }), { DB: env.DB, APP_BASE_URL: "https://memory.example", SETUP_TOKEN_HASH: tokenHash });
    const body = await response.json<{ options: { rp: { id: string }; authenticatorSelection: { userVerification: string } } }>();
    expect(response.status).toBe(200);
