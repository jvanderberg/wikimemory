...
describe("secret scanner", () => {
  ...
  it("does not flag ordinary prose", async () => {
    await expect(scanSecrets({ body: "A design decision about OAuth and SQLite." })).resolves.toEqual([]);
  });

  it("does not classify long URL paths or media filenames as secrets", async () => {
    const wordpressImage = "https://example.org/wp-content/uploads/2026/07/OakParkAssessmentReportA9v3K2m7Q8x1Z6n4P5r0T2w9Y7u3.jpg";
    const report = "https://www.rand.org/content/dam/rand/pubs/research_reports/RRA3000/RRA3123-1/RAND_RRA3123-1.pdf";
