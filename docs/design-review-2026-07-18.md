...
3. **Raw SQLite web import:** parsing and atomically ingesting a realistic SQLite file
   inside a Worker is not a sound V1 design. `llmwiki` migration is now a local CLI
   using validated idempotent chunks; web import accepts only bounded Wikimemory
   JSONL.
4. **OAuth compatibility profile:** the original contract named OAuth but did not
   select client registration behavior. V1 now requires DCR and explicit
...
- Normal and administrative MCP scope profiles are separate; V1 does not rely on
...
The owner explicitly removed import and automated `llmwiki` migration from V1. The
review's prohibition on raw SQLite upload remains; the proposed JSONL import and
chunked migration implementation are superseded. V1 provides exports only.
